...
async function scrapePlaystations() {
  ...
  await page.goto("https://www.coolblue.be/nl/consoles/playstation5", {
    waitUntil: "networkidle0",
  });

  const pageTitle = await page.$eval(".filtered-search__header h1", (element) =>
  ...
  fs.writeFile(
    ...
    (err) => {
      if (err) throw err;
      console.log(
        "filteredProducts array has been written to filteredProducts.json"
      );
    }
  );
